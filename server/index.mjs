...
app.post('/api/outline',async(req,res,next)=>{
  try{
    const {type,prompt,uploadId,contentMode='generate',audience,tone,language,visualStyle,deckStyle='auto',themeId='recykal-core',imageSource='mixed',artStyleId='auto',customArtStyle='',imageVariations=1,styleReferences=[],research=false,templateId,knowledgeIds=[]}=req.body||{};
    ...
    res.json(await generateOutline({type,prompt,parsedFile,contentMode,audience,tone,language,visualStyle,deckStyle,themeId,imageSource,artStyleId,customArtStyle,imageVariations,styleReferences,research,template,knowledge}));
  }catch(e){next(e)}
});

app.post('/api/generate',async(req,res,next)=>{
  try{
    const {type,prompt,uploadId,contentMode='generate',audience,tone,language,visualStyle,deckStyle='auto',themeId='recykal-core',imageSource='mixed',artStyleId='auto',customArtStyle='',imageVariations=1,styleReferences=[],approvedOutline=null,research=false,templateId,knowledgeIds=[]}=req.body||{};
    if(!['presentation','document','graphic'].includes(type)) return res.status(400).json({error:'Choose Presentation, Document, or Graphic.'});
    ...
    const project=await generateProject({type,prompt,parsedFile,contentMode,audience,tone,language,visualStyle,deckStyle,themeId,imageSource,artStyleId,customArtStyle,imageVariations,styleReferences,approvedOutline,research,template,knowledge});
...
app.post('/api/generate-stream',async(req,res,next)=>{
  ...
  try{
    ...
    const project=await generateProjectStream({type,prompt,parsedFile,contentMode,audience,tone,language,visualStyle,deckStyle,themeId,imageSource,artStyleId,customArtStyle,imageVariations,styleReferences,approvedOutline,research,template,knowledge,signal:generationController.signal},async event=>send(event));
  ...
  }catch(e){console.error(e);clearInterval(heartbeat);send({stage:'error',error:e.message||'Generation failed.'});res.end();}
});
