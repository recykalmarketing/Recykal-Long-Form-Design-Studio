...
export async function exportPdf(project, {review=false,profile='digital'}={}) {
  ...
  for(let pi=0;pi<project.pages.length;pi++){
    ...
    if(layout==='cover'){
      ...
      const {kicker,heading,sub}=titleBlocks(page);y=130;if(kicker){const ks=textSize(kicker,8.5);doc.fillColor(kicker.style?.textColor||T.secondary).font(textFont(kicker,fonts.bold)).fontSize(ks).text(kicker.text,M,y,{width:contentW,align:textAlign(kicker,'left'),lineGap:textLineGap(kicker,ks,1.2)});y=doc.y+12;}const hb=heading||{text:page.title};const hs=textSize(hb,isPresentation?36:(String(hb.text||'').length>78?27:String(hb.text||'').length>48?31:35));doc.fillColor(hb.style?.textColor||T.text).font(textFont(hb,fonts.bold)).fontSize(hs).text(hb.text||page.title,M,y,{width:contentW*.77,align:textAlign(hb,'left'),lineGap:textLineGap(hb,hs,1.08)});y=doc.y+15;if(sub){const ss=textSize(sub,12);doc.fillColor(sub.style?.textColor||T.muted).font(textFont(sub,fonts.regular)).fontSize(ss).text(sub.text,M,y,{width:contentW*.63,align:textAlign(sub,'left'),lineGap:textLineGap(sub,ss,1.35)});}
      ...
      const paras=page.blocks.filter(b=>b.type==='paragraph');if(paras.length){const pb=paras[0],ps=textSize(pb,8.8);doc.fillColor(pb.style?.textColor||'#E8EEF7').font(textFont(pb,fonts.regular)).fontSize(ps).text(pb.text,M,H*.67,{width:contentW*.52,align:textAlign(pb,'left'),lineGap:textLineGap(pb,ps,1.4)});}
      continue;
    }
    if(layout==='closing'){
...
export async function exportPptx(project, {review=false}={}) {
  ...
  for (let i=0;i<project.pages.length;i++) {
    ...
    for (const b of page.blocks) {
      if (b.type==='heading') {slide.addText(b.text||'',{x:0.5,y,w:7.8,h:0.75,...pptTextStyle(b,25),bold:b.style?.fontWeight!=null?Number(b.style.fontWeight)>=600:true,color:(b.style?.textColor||T.text).replace('#',''),margin:0});y+=0.90;}
      else if (b.type==='subheading') {slide.addText(b.text||'',{x:0.5,y,w:7.8,h:0.45,...pptTextStyle(b,13),color:(b.style?.textColor||T.dark).replace('#',''),margin:0});y+=0.57;}
      else if (b.type==='kicker') {slide.addText(b.text||'',{x:0.5,y,w:5,h:0.25,...pptTextStyle(b,8),bold:b.style?.fontWeight!=null?Number(b.style.fontWeight)>=600:true,color:(b.style?.textColor||T.secondary).replace('#',''),margin:0});y+=0.32;}
        ...
        const weights=tableColumnWeightsForExport(b),sumW=weights.reduce((a,v)=>a+v,0),colW=weights.map(v=>7.8*v/sumW);slide.addTable(rows,{x:0.5,y,w:7.8,h:Math.min(2.7,0.32*rows.length+0.4),fontFace:pptFont,fontSize:b.tableStyle?.density==='compact'?7.2:8,border:{type:'solid',color:'DCE3EA',pt:b.tableStyle?.verticalRules?0.7:0.35},margin:b.tableStyle?.density==='compact'?0.035:0.06,autoFit:false,colW});y+=Math.min(2.85,0.32*rows.length+0.55);
      }
      ...
      else if (['paragraph','quote','table'].includes(b.type)) {const t=blockText(b);slide.addText(t,{x:0.5,y,w:7.8,h:Math.min(1.45,0.32+Math.ceil(t.length/110)*0.23),...pptTextStyle(b,b.type==='quote'?13:10.5),italic:b.style?.italic??(b.type==='quote'),color:(b.style?.textColor||T.text).replace('#',''),margin:0});y+=Math.min(1.55,0.40+Math.ceil(t.length/110)*0.23);}
      if (y>6.5) break;
    }
