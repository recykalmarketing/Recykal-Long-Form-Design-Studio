...
export async function exportPdf(project, {review=false,profile='digital'}={}) {
  ...
  let physicalPage=0,pageOpen=false,runningHeader='';
  const publicationHeaderFor=(page={})=>{
    const blocks=page.blocks||[];
    const kicker=blocks.find(b=>b.type==='kicker')?.text||'';
    const heading=blocks.find(b=>b.type==='heading')?.text||page.title||'';
    const clean=v=>String(v||'').replace(/\s+/g,' ').replace(/\s*[—-]\s*continued\s*$/i,'').trim();
    const k=clean(kicker),h=clean(heading);
    if(/^(?:part|chapter|section)\b/i.test(k))return clean(`${k} · ${h}`).slice(0,96);
    if(/^(?:part|chapter|section)\b/i.test(h))return h.slice(0,96);
    if(page.layout==='section-opener'&&h)return h.slice(0,96);
    return '';
  };
  ...
  const addPhysicalPage=(layout='editorial',continued=false,{dark=false,surface=null}={})=>{if(pageOpen)footer();doc.addPage({size:mediaSize,margin:0});pageOpen=true;physicalPage++;const W=TRIM_W,H=TRIM_H;doc.rect(0,0,W,H).fill(dark?T.dark:T.background);if(surface&&!dark)drawPageSurface(surface,W,H);let top=dark?70:54;if(!dark){const autoHeader=!['cover','section-opener','closing'].includes(layout)?(master.headerText||runningHeader):'';if(physicalPage===1&&master.logoMode!=='none'&&layout!=='cover'){doc.image(logo,M,24,{fit:[110,30]});top=84;}else if(autoHeader){doc.fillColor(T.muted).font(fonts.regular).fontSize(7.4).text(autoHeader,M,29,{width:W-2*M-170,ellipsis:true});if(continued)doc.fillColor(T.muted).font(fonts.bold).fontSize(6.5).text('CONTINUED',W-M-90,30,{width:90,align:'right'});doc.strokeColor('#E6EBF1').lineWidth(.6).moveTo(M,54).lineTo(W-M,54).stroke();top=72;}}return top;};
  ...
  const drawVectorPlaceholder=(title,x,y,w,h)=>{const low=String(title||'').toLowerCase();doc.save();doc.roundedRect(x,y,w,h,8).fillOpacity(.55).fill(T.surface).fillOpacity(1);const pad=Math.max(16,w*.08),cx=x+w*.5,cy=y+h*.52;doc.strokeColor(T.primary).lineWidth(1.5).opacity(.7);if(/deposit|refund|return|recycl|circular|packag|waste/.test(low)){const pts=[[x+pad,cy],[cx,cy-h*.22],[x+w-pad,cy],[cx,cy+h*.22]],labels=['package','return','data','recycle'];for(let i=0;i<pts.length;i++){const [px,py]=pts[i],nx=pts[(i+1)%pts.length][0],ny=pts[(i+1)%pts.length][1];doc.moveTo(px,py).lineTo(nx,ny).stroke();drawIcon(labels[i],px-13,py-13,26,i%2?T.secondary:T.primary);doc.circle(px,py,21).strokeColor(i%2?T.secondary:T.primary).lineWidth(.8).stroke();}}else if(/data|research|evidence|performance|report|insight/.test(low)){const bw=(w-pad*2)/5;for(let i=0;i<5;i++){const bh=h*(.16+i*.055);doc.fillColor(i===4?T.secondary:T.primary).fillOpacity(.13+i*.025).roundedRect(x+pad+i*bw, y+h*.70-bh,bw*.58,bh,3).fill();}doc.fillOpacity(1);drawIcon('data',cx-18,y+h*.20,36,T.secondary);}else{drawPattern(x+pad,y+pad,w-pad*2,h-pad*2,T.secondary);drawIcon(low,cx-20,cy-20,40,T.primary);}doc.opacity(1).restore();};
  ...
  const drawToc=(page,block,startY)=>{const rows=(block?.tableRows||[]).filter(r=>r?.length>=2),contentW=TRIM_W-2*M,bottom=TRIM_H-footerH-24;let y=startY;const two=rows.length>22,cols=two?2:1,gap=28,colW=(contentW-gap*(cols-1))/cols,perCol=Math.ceil(rows.length/cols);for(let c=0;c<cols;c++){let yy=y;const slice=rows.slice(c*perCol,(c+1)*perCol);for(const row of slice){const label=String(row[0]??'').replace(/\.{2,}/g,' ').trim(),pg=String(row[1]??'').trim();const major=/^(PART\b|ANNEX|EXECUTIVE SUMMARY|TABLE OF CONTENTS|GLOSSARY|CONCLUSION)/i.test(label),chapter=/^CHAPTER\b/i.test(label);const fs=major?8.9:chapter?7.7:8.1,font=major?fonts.bold:chapter?fonts.bold:fonts.regular;doc.font(font).fontSize(fs);const h=Math.max(17,doc.heightOfString(label,{width:colW-44,lineGap:1.2})+7);if(yy+h>bottom)break;const x=M+c*(colW+gap);if(major){doc.fillColor(T.primary).fillOpacity(.055).roundedRect(x,yy-1,colW,h,3).fill().fillOpacity(1);}doc.fillColor(major?T.text:T.muted).font(font).fontSize(fs).text(label,x+4,yy+3,{width:colW-50,lineGap:1.2});doc.fillColor(major?T.primary:T.text).font(fonts.bold).fontSize(fs).text(pg,x+colW-40,yy+3,{width:36,align:'right'});doc.strokeColor('#E4E9EF').lineWidth(.45).moveTo(x+4,yy+h-1).lineTo(x+colW-4,yy+h-1).stroke();yy+=h;} }return bottom;};
  const drawGlossary=(page,block,startY)=>{const rows=(block?.tableRows||[]).filter(r=>r?.length>=2),contentW=TRIM_W-2*M,gap=25,colW=(contentW-gap)/2,bottom=TRIM_H-footerH-24;const density=rows.reduce((n,r)=>n+String(r[0]||'').length+String(r[1]||'').length,0),fs=density>2600?6.7:density>1900?7.1:7.5,termFs=fs+.45;let col=0,y=startY;const est=(r)=>{doc.font(fonts.regular).fontSize(fs);const dh=doc.heightOfString(String(r[1]||''),{width:colW,lineGap:1.8});doc.font(fonts.bold).fontSize(termFs);const th=doc.heightOfString(String(r[0]||''),{width:colW,lineGap:1.2});return th+dh+15;};const total=rows.reduce((n,r)=>n+est(r),0),target=total/2;let used=0;for(let i=0;i<rows.length;i++){const r=rows[i],h=est(r);if(col===0&&used>target*.92&&i<rows.length-1){col=1;y=startY;used=0;}if(y+h>bottom&&col===0){col=1;y=startY;used=0;}const x=M+col*(colW+gap);doc.fillColor(T.primary).font(fonts.bold).fontSize(termFs).text(String(r[0]||''),x,y,{width:colW,lineGap:1.2});y=doc.y+3;doc.fillColor(T.text).font(fonts.regular).fontSize(fs).text(String(r[1]||''),x,y,{width:colW,lineGap:1.8});y=doc.y+7;doc.strokeColor('#E1E8EE').lineWidth(.45).moveTo(x,y).lineTo(x+colW,y).stroke();y+=7;used+=h;}return Math.max(y,startY);};
  const drawEditorialSidebar=async(page,body,startY)=>{const contentW=TRIM_W-2*M,gap=20,leftW=contentW*.64,rightW=contentW-leftW-gap,bottom=TRIM_H-footerH-24;const paras=body.filter(b=>['paragraph','quote'].includes(b.type));const side=body.filter(b=>['bullets','stat','table'].includes(b.type));let ly=startY;for(const b of paras){const size=b.type==='quote'?11:9.2,font=b.type==='quote'?fonts.bold:fonts.regular;doc.fillColor(b.type==='quote'?T.dark:T.text).font(font).fontSize(size).text(b.type==='quote'?`“${b.text||''}”`:b.text||'',M,ly,{width:leftW,lineGap:b.type==='quote'?2.3:3});ly=doc.y+11;if(ly>bottom-36)break;}let y=startY,x=M+leftW+gap;doc.fillColor(T.primary).fillOpacity(.055).roundedRect(x-8,y-5,rightW+16,Math.min(430,TRIM_H-y-footerH-25),8).fill().fillOpacity(1);for(const b of side){if(b.type==='bullets'){doc.fillColor(T.text).font(fonts.bold).fontSize(8).text(b.label||'KEY POINTS',x,y,{width:rightW});y=doc.y+9;y=drawBullets(b.items||[],x,y,rightW,{icons:true,size:7.8})+8;}else if(b.type==='stat'){y=drawStats([b],x,y,rightW)+10;}else if(b.type==='table'){doc.fillColor(T.muted).font(fonts.regular).fontSize(7.2).text((b.tableRows||[]).slice(0,8).map(r=>r.join('  ·  ')).join('\n'),x,y,{width:rightW,lineGap:2});y=doc.y+8;}}return Math.max(ly,y);};
  ...
  for(let pi=0;pi<project.pages.length;pi++){
    ...
    const nextRunningHeader=publicationHeaderFor(page);if(nextRunningHeader)runningHeader=nextRunningHeader;
    ...
    if(layout==='section-opener'){
      const structural=(flowPage.blocks||[]).find(b=>b.type==='kicker'||b.type==='heading')?.text||page.title;const num=(String(structural).match(/(?:chapter|part|section)\s+([ivxlcdm\d]+)/i)||[])[1]||String(physicalPage).padStart(2,'0');doc.fillColor(T.primary).fillOpacity(.055).font(fonts.bold).fontSize(118).text(String(num).toUpperCase(),W*.58,H*.10,{width:W*.32,align:'right'}).fillOpacity(1);drawIcon(page.title,W*.76,H*.31,48,T.secondary);y=H*.31;y=drawHeading(flowPage,y,{width:contentW*.76,size:31});const body=flowPage.blocks.filter(b=>!['kicker','heading','subheading'].includes(b.type));if(body.length)await drawColumns({...page,blocks:body},Math.max(y,H*.49),body.reduce((n,b)=>n+String(b.text||'').length,0)>900?2:1);await drawFreeformBlocks(page,freeformBlocks);continue;
    }
    ...
    if(layout==='toc'){
      const tbl=body.find(b=>b.type==='table');if(tbl)y=drawToc(page,tbl,y);else await drawColumns(flowPage,y,2);
    }else if(layout==='glossary'){
      const tbl=body.find(b=>b.type==='table');if(tbl)y=drawGlossary(page,tbl,y);else await drawColumns(flowPage,y,2);
    }else if(layout==='message'){
      doc.fillColor(T.secondary).rect(M,y,4,72).fill();drawIcon('government policy',W-M-50,y+2,36,T.primary);const paras=body.filter(b=>b.type==='paragraph'),other=body.filter(b=>b.type!=='paragraph');if(paras.length){const lead=paras[0],ls=textSize(lead,11.2);doc.fillColor(T.dark).font(textFont(lead,fonts.regular)).fontSize(ls).text(lead.text,M+18,y,{width:contentW-82,lineGap:textLineGap(lead,ls,1.42)});y=doc.y+15;await drawColumns({...page,blocks:[...paras.slice(1),...other]},y,2);}else await drawColumns({...page,blocks:body},y,2);
    }else if(layout==='editorial-sidebar'){
      await drawEditorialSidebar(page,body,y);
    }else if(layout==='data-story'){
      const chart=body.find(b=>b.type==='chart'),stats=body.filter(b=>b.type==='stat'),bullets=body.find(b=>b.type==='bullets');if(stats.length)y=drawStats(stats.slice(0,4),M,y,contentW)+12;if(chart)y=drawChart(chart,M,y,contentW,210)+12;else if(bullets)y=drawProcess(page,bullets.items||[],y)+10;const paras=body.filter(b=>b.type==='paragraph');if(paras.length)await drawColumns({...page,blocks:paras},y,2);
    }else if(layout==='case-study'){
      const side=body.filter(b=>['stat','bullets','quote'].includes(b.type)),paras=body.filter(b=>b.type==='paragraph'),img=body.find(b=>b.type==='image');const leftW=contentW*.62,rightX=M+leftW+20,rightW=contentW-leftW-20;if(img)await drawImage(img,rightX,y,rightW,190);else{doc.fillColor(T.secondary).fillOpacity(.08).roundedRect(rightX,y,rightW,190,8).fill().fillOpacity(1);drawIcon(page.title,rightX+rightW/2-20,y+65,40,T.secondary);}let ly=y;for(const pb of paras){doc.fillColor(T.text).font(fonts.regular).fontSize(9.1).text(pb.text||'',M,ly,{width:leftW,lineGap:3});ly=doc.y+11;}let sy=y+205;for(const b of side){if(b.type==='stat')sy=drawStats([b],rightX,sy,rightW)+9;else if(b.type==='bullets')sy=drawBullets(b.items||[],rightX,sy,rightW,{icons:true,size:7.7})+8;else{doc.fillColor(T.dark).font(fonts.bold).fontSize(10).text(b.text||'',rightX,sy,{width:rightW,lineGap:2});sy=doc.y+8;}}
    }else if(layout==='summary'){
      const bullets=body.find(b=>b.type==='bullets'),stats=body.filter(b=>b.type==='stat');if(stats.length)y=drawStats(stats.slice(0,4),M,y,contentW)+14;if(bullets){const items=bullets.items||[],gap=12,cw=(contentW-gap)/2;items.forEach((it,i)=>{const cx=M+(i%2)*(cw+gap),cy=y+Math.floor(i/2)*74;doc.roundedRect(cx,cy,cw,62,7).fillOpacity(.06).fill(i%2?T.secondary:T.primary).fillOpacity(1);doc.fillColor(T.text).font(fonts.bold).fontSize(8.2).text(it,cx+12,cy+12,{width:cw-24,lineGap:1.5});});y+=Math.ceil(items.length/2)*74;}const paras=body.filter(b=>b.type==='paragraph');if(paras.length)await drawColumns({...page,blocks:paras},y+4,2);
    }else if(layout==='profile'){
      const tbl=body.find(b=>b.type==='table'),stats=body.filter(b=>b.type==='stat'),paras=body.filter(b=>b.type==='paragraph');if(stats.length)y=drawStats(stats.slice(0,4),M,y,contentW)+12;if(tbl)y=await drawTable(page,tbl,y);if(paras.length)await drawColumns({...page,blocks:paras},y+8,2);
      ...
      const bullets=body.find(b=>b.type==='bullets');if(bullets)y=drawProcess(page,bullets.items||[],y)+8;else{drawVectorPlaceholder(page.title,M,y,contentW,145);y+=158;}const paras=body.filter(b=>b.type==='paragraph');if(paras.length)await drawColumns({...page,blocks:paras},y,2);
