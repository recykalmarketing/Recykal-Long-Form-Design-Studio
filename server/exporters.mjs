...
export async function exportPdf(project, {review=false,profile='digital'}={}) {
  ...
  for(let pi=0;pi<project.pages.length;pi++){
    ...
    if(layout==='cover'){
      ...
      const {kicker,heading,sub}=titleBlocks(flowPage);y=130;if(kicker){const ks=textSize(kicker,8.5);doc.fillColor(kicker.style?.textColor||T.secondary).font(textFont(kicker,fonts.bold)).fontSize(ks).text(kicker.text,M,y,{width:contentW,align:textAlign(kicker,'left'),lineGap:textLineGap(kicker,ks,1.2)});y=doc.y+12;}const hb=heading||{text:page.title};const hs=textSize(hb,isPresentation?36:(String(hb.text||'').length>78?27:String(hb.text||'').length>48?31:35));doc.fillColor(hb.style?.textColor||T.text).font(textFont(hb,fonts.bold)).fontSize(hs).text(hb.text||page.title,M,y,{width:contentW*.77,align:textAlign(hb,'left'),lineGap:textLineGap(hb,hs,1.08)});y=doc.y+15;if(sub){const ss=textSize(sub,12);doc.fillColor(sub.style?.textColor||T.muted).font(textFont(sub,fonts.regular)).fontSize(ss).text(sub.text,M,y,{width:contentW*.63,align:textAlign(sub,'left'),lineGap:textLineGap(sub,ss,1.35)});}
      const img=flowPage.blocks.find(b=>b.type==='image');const bx=W*.56,by=135,bw=W*.37,bh=H*.34;if(!(await drawImage(img,bx,by,bw,bh)))drawVectorPlaceholder(page.title,bx,by,bw,bh);drawPattern(M,H*.68,contentW,H*.21,'#71D9BB');
      const paras=flowPage.blocks.filter(b=>b.type==='paragraph');if(paras.length){const pb=paras[0],ps=textSize(pb,8.8);doc.fillColor(pb.style?.textColor||'#E8EEF7').font(textFont(pb,fonts.regular)).fontSize(ps).text(pb.text,M,H*.67,{width:contentW*.52,align:textAlign(pb,'left'),lineGap:textLineGap(pb,ps,1.4)});}
      await drawFreeformBlocks(page,freeformBlocks);
      continue;
    }
    if(layout==='closing'){
      drawPattern(M,H*.42,contentW,H*.34,'#79DFC2');y=H*.22;drawHeading(flowPage,y,{width:contentW*.75,dark:true,size:30});const paras=flowPage.blocks.filter(b=>b.type==='paragraph');if(paras[0])doc.fillColor('#DDE9F5').font(fonts.regular).fontSize(10).text(paras[0].text,M,H*.34,{width:contentW*.62,lineGap:2.5});await drawFreeformBlocks(page,freeformBlocks);continue;
    }
