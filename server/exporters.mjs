...
export async function exportPdf(project, {review=false,profile='digital'}={}) {
  ...
  const drawColumns=async(page,startY,cols=2)=>{const W=TRIM_W,contentW=W-2*M,gap=18,colW=(contentW-gap*(cols-1))/cols;let col=0,y=startY;const textBlocks=page.blocks.filter(b=>['paragraph','bullets','quote','stat','image'].includes(b.type));for(const b of textBlocks){if(b.type==='stat'){if(col!==0){col=0;y=Math.max(y,startY)+8;}y=drawStats([b],M,y,contentW)+12;continue;}if(b.type==='image'){if(col!==0){col=0;y=Math.max(y,startY)+8;}const h=210;const ok=await drawImage(b,M,y,contentW,h);if(!ok)drawVectorPlaceholder(page.title,M,y,contentW,h);y+=h+12;continue;}if(b.type==='bullets'){const x=M+col*(colW+gap);y=drawBullets(b.items,x,y,colW,{icons:true,size:8.9});col++;if(col>=cols){col=0;y+=6;}continue;}let remaining=b.type==='quote'?`“${b.text||''}”`:b.text||'';while(remaining){const x=M+col*(colW+gap),available=(TRIM_H-footerH-28)-y;const font=b.type==='quote'?fonts.italic:fonts.regular,size=b.type==='quote'?12:9.4,lineGap=3;const [part,rest]=splitToFit(remaining,colW,available,font,size,lineGap);doc.fillColor(b.type==='quote'?T.dark:T.text).font(font).fontSize(size).text(part,x,y,{width:colW,lineGap});y=doc.y+10;remaining=rest;if(remaining||y>TRIM_H-footerH-50){col++;if(col>=cols){col=0;y=addPhysicalPage(page.layout,true);}else y=startY;}}col++;if(col>=cols){col=0;y+=3;}else y=startY;}return y;};
  ...
  for(let pi=0;pi<project.pages.length;pi++){
    ...
    if(layout==='table'){
      ...
      await drawColumns(page,y,layout==='two-column'?2:2);
    }
  }
